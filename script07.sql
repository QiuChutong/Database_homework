...
DROP VIEW Company_In_NY;
DROP VIEW Unit_Price_By_Store;
...
INSERT INTO Store (StoreID, CompanyTIN, State, Manager) VALUES (548, 448573597, 'MA', Null);
...
INSERT INTO Store (StoreID, CompanyTIN, State, Manager) VALUES (366, 976382037, 'PA', Null);
...
INSERT INTO Store (StoreID, CompanyTIN, State, Manager) VALUES (171, 315145601, 'NJ', Null);
...
INSERT INTO R VALUES (2, 'FIRST');
INSERT INTO R VALUES (2, 'FIRST');
INSERT INTO R VALUES (2, 'SECOND');
INSERT INTO R VALUES (1, 'FIRST');
INSERT INTO R VALUES (2, 'SECOND');
INSERT INTO R VALUES (1, 'FIRST');
INSERT INTO R VALUES (1, 'FIRST');

INSERT INTO S VALUES (1, 'SECOND');
INSERT INTO S VALUES (1, 'SECOND');
INSERT INTO S VALUES (1, 'SECOND');
INSERT INTO S VALUES (1, 'THIRD');
INSERT INTO S VALUES (2, 'FIRST');
INSERT INTO S VALUES (2, 'THIRD');
INSERT INTO S VALUES (3, 'THIRD');
INSERT INTO S VALUES (3, 'FIRST');
...
-- ANSWER1
CREATE TABLE TEMP1
AS SELECT DISTINCT Store.StoreID, Store.Manager, Sells.Price*Sells.Quantity AS Gross
FROM Store, Sells
WHERE Store.StoreID=Sells.StoreID AND Store.CompanyTIN=Sells.CompanyTIN;

CREATE TABLE ANSWER1
AS SELECT DISTINCT StoreID, Manager
FROM TEMP1
WHERE Gross>=1000
ORDER BY StoreID ASC, Manager ASC;
...
-- ANSWER2
CREATE TABLE TEMP2
AS SELECT DISTINCT E.StoreID, E.CompanyTIN, E.UPC
FROM Store T, Sells E
WHERE T.StoreID=E.StoreID AND T.Manager IS NOT NULL;

CREATE TABLE ANSWER2
AS SELECT DISTINCT StoreID, CompanyTIN, COUNT(UPC) AS NumberOfProducts
FROM TEMP2
GROUP BY StoreID, CompanyTIN
HAVING COUNT(UPC)>=3
ORDER BY StoreID ASC, CompanyTIN ASC, NumberOfProducts ASC;
...
-- ANSWER3
CREATE TABLE TEMP3
AS SELECT DISTINCT UPC
FROM Sells
WHERE StoreID=366 AND CompanyTIN=976382037;

CREATE TABLE TEMP4
AS SELECT DISTINCT Sells.StoreID, Sells.CompanyTIN, Sells.UPC
FROM Sells, TEMP3
WHERE Sells.UPC=TEMP3.UPC;

CREATE TABLE TEMP5
AS SELECT DISTINCT TEMP4.StoreID, TEMP4.CompanyTIN, TEMP3.UPC
FROM TEMP4, TEMP3;

CREATE TABLE TEMP6
AS SELECT DISTINCT StoreID, CompanyTIN, UPC
FROM TEMP5
MINUS
SELECT *
FROM TEMP4;
...
FROM TEMP4
MINUS
SELECT DISTINCT StoreID, CompanyTIN
FROM TEMP6
ORDER BY StoreID ASC, CompanyTIN ASC;
...
-- ANSWER6
INSERT INTO Product (UPC, Name)
VALUES (196178,'Pepper');
...
-- ANSWER7
CREATE VIEW Company_In_NY
AS SELECT Company.CompanyTIN AS CompanyTIN, Earnings, State
FROM Company, Store
WHERE Company.CompanyTIN = Store.CompanyTIN AND Store.State = 'NY';

INSERT INTO Company_In_NY
VALUES (121370175, NULL, 'NY');
...
-- ANSWER8
CREATE VIEW Unit_Price_By_Store
AS SELECT CompanyTIN, StoreID, UPC, Price
FROM Sells;

INSERT INTO Unit_Price_By_Store
VALUES (315145601, 171, 126901, 30);
